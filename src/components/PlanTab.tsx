/* --------------------------------------------------------------------------
   ...
   Gating: cần Knowdell + Holland (đọc duy nhất từ /api/profile/summary)
-------------------------------------------------------------------------- */
"use client"
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        // Knowdell: cần có summary
        const hasKnowdell =
          typeof p?.knowdell_summary === "string" &&
          p.knowdell_summary.trim().length > 0

        // Holland: lấy từ các key phổ biến; nếu không có, dùng proxy suggested_jobs
        const hasHolland =
          ...
          (Array.isArray(p?.holland_scores) && p.holland_scores.length > 0) ||
          (Array.isArray(p?.suggested_jobs) && p.suggested_jobs.length > 0) // proxy an toàn
